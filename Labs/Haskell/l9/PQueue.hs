...
class (Ord a) => PQueue pq a where
    ...
    toList :: pq a -> [(Prio, a)]
    toList pQ = if (isEmpty pQ) then [] else (fromJust $ top pQ) : (toList $ pop pQ)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert a (LPQ pq)
      | null pq = LPQ [a]
      | fst a >= fst (head pq) = (LPQ (a : pq))
      | otherwise = (LPQ ((head pq) : (toList (insert a (LPQ (tail pq))))))

    top (LPQ pq) = if (null pq) then Nothing else (Just (head pq))

    pop (LPQ pq) = (LPQ (tail pq))

    isEmpty (LPQ pq) = if (null pq) then True else False 
...
merge (Empty 0) pq = pq
merge pq (Empty 0) = pq
merge (Node rank1 (p1, a1) left1 right1) (Node rank2 (p2, a2) left2 right2) 
  | p1 <= p2 = (merge (Node rank2 (p2, a2) left2 right2) (Node rank1 (p1, a1) left1 right1))
  | rank left1 >= (rank (merge right1 (Node rank2 (p2, a2) left2 right2))) = (Node (rank (merge right1 (Node rank2 (p2, a2) left2 right2)) + 1) (p1, a1) left1 (merge right1 (Node rank2 (p2, a2) left2 right2)))
  | otherwise = (Node ((rank left1) + 1) (p1, a1) (merge right1 (Node rank2 (p2, a2) left2 right2)) left1)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty 0) = []
inorder (Node rank nodeV left right) = inorder left ++ [nodeV] ++ inorder right
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    top pq = if (isEmpty pq) then Nothing else (Just (nodeVal pq))

    pop = undefined

    size pq = (length (toList pq))
