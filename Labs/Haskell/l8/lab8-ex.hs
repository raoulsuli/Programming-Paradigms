...
normalizeV (V vx vy vz) = (V (vx / (lengthV (V vx vy vz))) (vy / (lengthV (V vx vy vz))) (vz / (lengthV (V vx vy vz))))
...
orthogonalV v = if (dotV (head v) (head (tail v))) == 0 then True else False 
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = Empty | ConsElem a (SList a) | ConsList (SList a) (SList a) deriving Show

emptySList :: SList a
emptySList = Empty 

consElem :: a -> SList a -> SList a
consElem a b = ConsElem a b

consList :: SList a -> SList a -> SList a
consList a b = ConsList a b

headSList :: SList a -> SList a
headSList Empty = Empty
headSList (ConsList a b) = a
headSList (ConsElem a b) = consElem a emptySList

tailSList :: SList a -> SList a
tailSList Empty = Empty
tailSList (ConsList a b) = b
tailSList (ConsElem a b) = b

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual Empty Empty = True
deepEqual (ConsElem x a) (ConsElem y b) = if (x == y) then deepEqual a b else False
deepEqual (ConsList a b) (ConsList c d) = if (deepEqual a c) then deepEqual b d else False
deepEqual _ _ = False 

flatten :: SList a -> [a]
flatten Empty = []
flatten (ConsElem a b) =  [a] ++ flatten b
flatten (ConsList a b) = flatten a ++ flatten b
...
insertElem (BSTNode a left right) b = if (b <= a) then (BSTNode a (insertElem left b) right) else (BSTNode a left (insertElem right b)) 

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil b = Nothing
findElem (BSTNode a left right) b 
	| a == b = Just b 
	| b < a = findElem left b
	| otherwise = findElem right b

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree BSTNil x y = Nothing
subTree (BSTNode curr left right) x y
	| findElem (BSTNode curr left right) x == Nothing || findElem (BSTNode curr left right) y == Nothing = Nothing
	| curr >= x && curr <= y = Just (BSTNode curr left right)
	| curr > x && curr > y = subTree left x y
	| curr < x && curr < y = subTree right x y

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode curr left right) = [curr] ++ inorder left ++ inorder right
...
foldlTree f x (Node value children) = undefined--foldl f (x ++ value) children
