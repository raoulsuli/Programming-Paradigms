...
(define (bases-n n Bases num)
  (if (null? Bases)
      num
      (if (palindrome? (num->base n (car Bases)))
          (bases-n n (cdr Bases) (+ num 1))
          (bases-n n (cdr Bases) num))))
...
(define (first-b-pal start b)
  (if (>= (bases-n start '(2 3 4 5 6 7 8 9 10) 0) b)
      start
...
(define (longest-palindrome n)
   (find (num->base n 10) '() 0))

(define (find n answer num)
  'undefined)
