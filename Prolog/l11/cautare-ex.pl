...
safe(_/_/_, []):- true, !.
safe(X/Y/S, [_/_/S1 | Rest]):- S\= S1, safe(X/Y/S, Rest), !.
safe(X/Y/S, [X1/Y1/_ | Rest]):- X \= X1, Y \= Y1, safe(X/Y/S, Rest).
...
% problemă.
safeMisionari(X, Y) :- X >= Y, !, true. 
safeMisionari(0, _):- true, !.
...
% S1.
next_state(misionari, state(Mal1, NM1e, NC1e, NM1v, NC1v), state(Mal2, NM2e, NC2e, NM2v, NC2v)) :-
	boat(X,Y),
	Mal1 == est,
	\+(X > NM1e), \+(Y > NC1e),
	NM2e is NM1e - X,
	...
	NM2v is NM1v + X,
	NC2v is NC1v + Y,
	safeMisionari(NM2e, NC2e),
	safeMisionari(NM2v, NC2v),
	opus(Mal1, Mal2).
