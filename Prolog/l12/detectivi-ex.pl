...
au_arma(Arma, ListaNume) :- bagof(X, inarmat(X, Arma), ListaNume).
...
arme_bmw(ListaArme) :- findall(Arma, suspect(_:bmw:Arma), ListaArme).
...
arme_marca(Marca, ListaArme) :- findall(Arma, (suspect(_:Marca:Arma)), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, (suspect(_:Marca:_), arme_marca(Marca, [_])), ListaMarci).
...
suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma), arme_marca(Marca, ListaArme), forall(member(Arme, ListaArme), au_arma(Arme, [_ , _| _])).
