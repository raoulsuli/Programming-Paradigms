...
(define-struct bird-state (x y v-y) #:transparent)
(define-struct game-state (bird-state pipes abilities variables score) #:transparent)

(define-struct pipe (gap-y x) #:transparent)
(define-struct variables (gravity momentum scroll-speed) #:transparent)
(define-struct abilities(visible active) #:transparent)

(define initial-bird (bird-state bird-x bird-initial-y 0.0))

(define initial-variables (variables initial-gravity initial-momentum initial-scroll-speed))
(define initial-abilities (abilities '() '()))
(define initial-pipe (pipe (+ added-number (random random-threshold)) scene-width))

(define initial-state (game-state initial-bird (list initial-pipe) initial-abilities initial-variables 0))
...
(define (next-state-bird bird gravity)
  (struct-copy bird-state bird [y (+ (bird-state-y bird) (bird-state-v-y bird))] [v-y (+ (bird-state-v-y bird) gravity)]))
...
(define (change current-state pressed-key)
  (if(equal? pressed-key " ")
     (struct-copy game-state current-state [bird-state (next-state-bird-onspace (get-bird current-state) initial-momentum)])
     current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (x) (if(null? x)
                      null
                      (struct-copy pipe x [x (- (get-pipe-x x) scroll-speed)]))) pipes))
...
(define (clean-pipes pipes)
  (filter (lambda (x) (>= (+ (get-pipe-x x) pipe-width) 0)) pipes))
...
(define (add-more-pipes pipes)
  (if(>= (length pipes) no-pipes)
     pipes
     (append pipes (list (struct-copy pipe (lastElem pipes)
                                                      [gap-y (+ added-number (random random-threshold))]
                                                      [x (+ pipe-width pipe-gap (get-pipe-x (lastElem pipes)))])))))
...
(define (invalid-state? state)
  (or (check-pipe-collisions (get-bird state) (get-pipes state)) (check-ground-collision (get-bird state))))
...
(define (check-pipe-collisions bird pipes)
 (if (null? pipes)
  #f
  (or (check-collision-rectangles (make-posn (bird-state-x bird) (get-bird-y bird) ) (make-posn (+ (bird-state-x bird) bird-width) (+ (get-bird-y bird) bird-height) )
                                   (make-posn (get-pipe-x (car pipes)) 0) 
                                   (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (pipe-gap-y (car pipes)) )) 
  (check-collision-rectangles (make-posn (bird-state-x bird) (get-bird-y bird) ) (make-posn (+ (bird-state-x bird) bird-width) (+ (get-bird-y bird) bird-height) )
                                   (make-posn (get-pipe-x (car pipes)) (+ (pipe-gap-y (car pipes)) pipe-self-gap)) 
...
(define (next-state state)
  (struct-copy game-state state [bird-state (next-state-bird (get-bird state) initial-gravity)]
               [pipes (next-state-pipes (get-pipes state) initial-scroll-speed)] [score (+ (get-score state) 0.1)] ))


;[variables (next-variables initial-variables (get-abilities state))]
...
(define (draw-frame state)
  (place-images (list bird-image ground-image (score-to-image (get-score state)))
                (list (make-posn (+ (bird-state-x (get-bird state)) (quotient bird-width 2)) (+ (get-bird-y (get-bird state)) (quotient bird-height 2)))
...
(define (place-pipes pipes scene)
  (if(null? pipes)
     scene
     (place-pipes (cdr pipes) (place-images (list pipe-image pipe-image)
                                            (list (make-posn (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (- (pipe-gap-y (car pipes)) (quotient pipe-height 2)) )
                                                  (make-posn (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (+ (pipe-gap-y (car pipes)) pipe-self-gap (quotient pipe-height 2)) ))
                                            scene))))
...
(define fast-ability (ability_str (hourglass "mediumseagreen") 10 null (max 5 (- (variables-scroll-speed initial-variables) 1))))

; Abilitatea care va încetini timpul va dura 30 de secunde, va avea imaginea (hourglass "tomato")
; va avea inițial poziția null si va modifica scrolls-speed dupa formulă
; scroll-speed = scroll-speed + 1
(define slow-ability (ability_str (hourglass "tomato") 30 null (+ 1 (variables-scroll-speed initial-variables))))

; lista cu toate abilităţile posibile în joc
(define ABILITIES (list slow-ability fast-ability))


(define get-variables initial-variables)
(define get-variables-gravity (variables-gravity get-variables))
(define get-variables-momentum (variables-momentum get-variables))
(define get-variables-scroll-speed (variables-scroll-speed get-variables))
...
(define (clean-abilities abilities)
  (filter (lambda (x) (>= (posn-x x) 0)) abilities))
...
(define (move-abilities abilities scroll-speed)
  (map (lambda (x) (if(null? x)
                      null
                      (struct-copy ability_str x [pos (make-posn (- (posn-x x) scroll-speed) (posn-y x))]))) (get-abilities-visible abilities)))
...
(define (time-counter abilities)
	(filter (lambda (x) (<= (get-ability-time x) 0)) (struct-copy ability_str abilities [time (- (get-ability-time abilities) 1)] )))
...
(define (next-abilities-visible visible scroll-speed)
	(fill-abilities (clean-abilities (move-abilities (time-counter visible) scroll-speed)) DISPLAYED_ABILITIES visible))
...
(define (next-abilities abilities bird scroll-speed)
	(filter (lambda (x) (check-collision-rectangles (make-posn (bird-state-x bird) (get-bird-y bird)) (make-posn (+ (bird-state-x bird ) bird-width) (+ (get-bird-y bird) bird-height))
                             (get-ability-pos x) (make-posn (+ (posn-x (get-ability-pos x)) 40) (+ (posn-x (get-ability-pos x)) 40)))) abilities))
...
(define (place-visible-abilities abilities scene)
  (if(null? (abilities-visible abilities))
     scene
     (place-visible-abilities (cdr (cdr (abilities-visible abilities))) (place-image (get-ability-image (car (abilities-visible abilities)))
                                                                                     (posn-x (get-ability-pos (car (abilities-visible abilities))))
                                                                                     (posn-y (get-ability-pos (car (abilities-visible abilities)))) scene))))
