...
revista(titlu('National Geographic'), anul(2019), luna('septembrie')).

%% predicate de folosit în interogări
in_biblioteca(X - Y) :- carte(titlu(X), autor(Y)).
in_biblioteca(X - Y/Z) :- revista(titlu(X), anul(Z), luna(Y)).
titlu(X) :- in_biblioteca(X - _).
titlu_struct(X) :- carte(X, _) ; revista(X, _, _).
...
last_(X,L) :- concat(_, [X], L).
del22(L,Rez) :- concat([_,_|Rez], [_,_], L).
