...
myConcat([], L, L).
myConcat([X|Res], L, [X|Rez]):- myConcat(Res, L, Rez).
...
myReverse([],[]).
myReverse([X|Res],L):- myReverse(Res, Y), myConcat(Y, [X], L).
...
myReverseAcc([X|Rest], Acc, Result):- myReverseAcc(Rest, [X|Acc], Result).
...
extract([X | Rest] , 0, Y, [X |Result]):- 
  Yupdate is Y - 1, 
  extract(Rest, 0, Yupdate, Result).

extract([_ | Rest], X, Y, Result):-
  Xupdate is X - 1, 
  Yupdate is Y - 1, 
  extract(Rest, Xupdate, Yupdate, Result).
...
factorial2(0,1).
factorial2(X, Y):- nonvar(X), !, factorial(X, Y).
factorial2(X, Y):- factorial2(Xrez, Yrez), X is Xrez + 1, Y is Yrez * X.
...
sublista([],[]).
sublista([_ | Rest], List):- sublista(Rest, List).
sublista([Head | Rest], [Head | Rest1]):- sublista(Rest, Rest1).
...
descendantOf(X,Y):- nod(X), nod(Y), arc(Z,X), descendantOf(Z,Y).
