...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg (get-line-segment)) (x (get-start-point seg)) (y (get-end-point seg)) (dist1 (- (car y) (car x))) (dist2 (- (cdr y) (cdr x))))
...
(define (distance p1 p2)
  (compute-length (lambda () (cons p1 p2)) car cdr))
...
(define (compute-f-with-step f a b step)
  (let iter ((b  b) (result null)) (if (> a b)
                                       result
...
(define (num-concat x y)
  (let ((+ string-append) (x (number->string x)) (y (number->string y)))
    (string->number (+ x y))));; Nu stergeți această linie.
...
(define (compute-perimeter points)
  (let ((p1 (car points)))
    (let iter ((p1 p1) (points points))(if (= (length points) 1)
        (distance (car points) p1)
        (+ (distance (car points) (cadr points)) (iter p1 (cdr points)))))))
...
(define (3-sequence-max numbers separator)
  (let*-values (((split) (lambda (value) (not (equal? value separator))))
